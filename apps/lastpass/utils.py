...
class LastPass_YUANLAI(LastPassBase):
    def __init__(self,**kwargs):
        ...
        self.secret = "5e5e5635736a06c8ca77e8131053a3b4"
        self.businessId = "1590"
...
class LastPass_ALLWIN(LastPassBase):
    def __init__(self,**kwargs):
        ...
        #生产环境
        self.create_order_url="http://allwin6666.com/api_new/business/create_order"
    ...
    def run(self):
        ...
        self.data.setdefault('paytypeid','21')
    ...
    def call_run(self):
        ...
        if not self.data['sign'] != sign:
            raise PubErrorCustom("签名不正确")


        if self.data.get('status')=='00':
            try:
                order = Order.objects.select_for_update().get(ordercode=self.data.get("ordercode"))
            except Order.DoesNotExist:
                raise PubErrorCustom("订单号不正确!")
